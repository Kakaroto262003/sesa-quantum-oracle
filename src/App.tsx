...
const SPATIAL_GEODATA_DICTIONARY: { [key: string]: { lat: number; lng: number; url: string } } = {
  ...
  "WAMENA": { lat: -4.0950, lng: 138.9460, url: "https://maps.google.com/maps?q=loc:-4.0950,138.9460&t=k&z=14&output=embed" }
};
...
export default function App() {
  ...
  const handleExportData = () => {
    if (logs.length === 0) return alert('🚨 EXPORT DENIED: EMPTY REPOSITORY.');
    let csvContent = "ID;TIMESTAMP;IDENTITY;COORDINATES;CATEGORY;AI_CONFIDENCE;POTENTIAL;INTEGRITY\n";
    logs.forEach((node) => {
      csvContent += `#AF${node.id};${node.time};${node.label};"${node.coord}";${node.kategori};${node.confidence}%;${node.potential};${node.integrity}\n`;
    });
                ...
                {selectedNode && (
                  ...
                  >
                    <div className="bg-zinc-900/40 border-2 border-zinc-800 rounded-lg p-3 space-y-2.5 text-xs">
                      <div className="flex justify-between border-b border-zinc-800/40 pb-1.5"><span className="text-zinc-400 font-bold font-sans">BUSINESS_NAME:</span><span className="text-white font-black font-sans text-[10px] break-all text-right max-w-[150px] uppercase">{selectedNode.label}</span></div>
